...
var newarr = ['Marcos', '13/05', 73, 1.85];
...
console.log(ceara ? 'Ceará está incluído!' : 'Ceará não está incluído ;(')
...
var map = newBrasil.map(function(item, index){
	return{
	id : item.id++,
	estado : item.estado + '. estado pertence ao brasil'
	};
})
...
*/
var par = map.filter(function(item, index){
	return item.id % 2 === 0;
})
